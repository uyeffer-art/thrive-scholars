      ...
      {unconfirmed.length > 0 && (
        <div className="mb-6 bg-amber-50 border border-amber-200 rounded-xl p-4">
          ...
          <p className="text-xs text-amber-700">
            Did these sessions happen? Please mark each one as Held or Missed so we can track your progress.
          </p>
        </div>
      )}
              ...
              <div key={i.id} className={`bg-white rounded-xl border p-5 ${needsConfirmation ? 'border-amber-300 ring-1 ring-amber-200' : 'border-gray-200'}`}>
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">
                      with {i.volunteers?.profiles?.first_name} {i.volunteers?.profiles?.last_name}
                    </p>
                ...
                {i.status === 'held' && (
                  <FeedbackForm
                    interactionId={i.id}
                    role="scholar"
                    existingRating={i.scholar_rating}
                    existingNotes={i.scholar_notes}
                  />
                )}
              </div>
            )
